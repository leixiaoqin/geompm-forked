...
class BodyReader(object):
    ...
    def set_element_calLength(self, scene: myScene, bodyID, psize):
        if self.sims.shape_function == "Linear":
            ...
            #LXQ
            scene.element.calLength[bodyID] = psize[0]
        ...
        if DictIO.GetAlternative(template, "Restart", False):
            ...
            if self.sims.coupling or self.sims.neighbor_detection:
                kernel_rebulid_particle_coupling(particle_number, scene.particle, scene.is_rigid,
                                                 ...
                                                 DictIO.GetEssential(particle_info, "k_factor"), #LXQ
            ...
            #LXQ don't know this variable and comment it at the moment
            ...
            #print("��ȡ�� normal ֵ:", normal)
            #kernel_set_particle_normal_(scene.particle, int(scene.particleNum[0]), particle_num, normal)
            #########################################################################################################
        else:
            bodyID = DictIO.GetEssential(template, "BodyID")
            self.check_bodyID(scene, bodyID)
            ...
            particle_num = coords.shape[0]
            #LXQ
            self.set_element_calLength(scene, bodyID, psize)
            ...
            #LXQ
            #print("shapelength:")
            #print(scene.element.calLength[bodyID])

            #########################################################################################################
            #LXQ
            #normal = DictIO.GetEssential(particle_cloud, "normal")        
            normal = np.array(DictIO.GetAlternative(particle_cloud, "normal", [0., 0., 1.]))
            if self.sims.contact_detection:
                kernel_set_particle_normal_(scene.particle, int(scene.particleNum[0]), particle_num, normal)

            k_factor = DictIO.GetAlternative(template, "k_factor", 1.0)
            #########################################################################################################
            scene.particleNum[0] += particle_num
