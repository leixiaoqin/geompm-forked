...
#LXQ
@ti.kernel
def kernel_set_particle_normal_(particle: ti.template(), particleNum: int,  particle_num: int, normal: ti.types.ndarray()):
    for np in range(particle_num):
        particle[particleNum + np].normal = vec3f(normal[np, 0],normal[np, 1], normal[np, 2])
############################################################################################################################################
