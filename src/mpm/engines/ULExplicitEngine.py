...
class ULExplicitEngine(Engine):
    ...
    def choose_boundary_constraints(self, sims: Simulation, scene: myScene):
        ...
        if int(scene.velocity_list[0]) > 0:
            print("velocity_list: ", int(scene.velocity_list[0]))
    ...
    def calculate_precontact(self, sims: Simulation, scene: myScene):
        ...
        #LXQ
        ################################################################################################################
        if scene.norm_method == "DTM":
            if sims.current_step < 2:   kernel_calc_contact_normal_dtm(scene.element.grid_nodes, int(scene.particleNum[0]), scene.node, scene.particle, scene.element.LnID, scene.element.shape_fn, scene.element.node_size, scene.is_rigid)
        ################################################################################################################
        if scene.norm_method == "MassWeighted":
    ...
    def compute_contact_force(self, sims: Simulation, scene: myScene):
        ...
        ################################################################################################################
        if scene.norm_method == "DTM":
            kernel_calc_friction_contact_dtm(scene.mass_cut_off, scene.mu, scene.element.contact_position_offset, sims.dt, sims.current_time, scene.is_rigid, scene.node, scene.particle)
        ################################################################################################################
        if scene.norm_method == "MassWeighted":
    ...
    def compute_nodal_kinematics_apic(self, sims: Simulation, scene: myScene):
        #LXQ
        print("compute_nodal_kinematics_apic")
        #kernel_mass_momentum_p2g(scene.element.grid_nodes, int(scene.particleNum[0]), scene.node, scene.particle, scene.element.LnID, scene.element.shape_fn, scene.element.node_size)
        kernel_momentum_apic_p2g(scene.element.grid_nodes, int(scene.particleNum[0]), scene.node, scene.element.nodal_coords, scene.particle, scene.element.LnID, scene.element.shape_fn, scene.element.node_size)
    ...
    def compute_stress_strain_apic(self, sims: Simulation, scene: myScene):
        #LXQ
        print("compute_stress_strain_apic")
        kernel_compute_stress_strain_apic(scene.element.grid_nodes, sims.dt, int(scene.particleNum[0]), scene.node, scene.element.nodal_coords, scene.particle, scene.material.matProps, scene.material.stateVars,
                                          scene.element.LnID, scene.element.dshape_fn, scene.element.node_size)    
    ...
    def absorbing_constraints(self, sims: Simulation, scene: myScene):
        ...
        apply_absorbing_constraint_p(int(scene.particleNum[0]), scene.particle, scene.material)
    ...
    def compute_particle_kinematics(self, sims: Simulation, scene: myScene):
        ...
        # LXQ
        kernel_kinemaitc_g2p(scene.element.grid_nodes, sims.alphaPIC, sims.particle_damping, sims.dt, int(scene.particleNum[0]), scene.node,
                             scene.particle, scene.element.LnID, scene.element.shape_fn, scene.element.node_size)
    ...
    def mls_updating(self, sims: Simulation, scene: myScene):
        ...
        self.compute_grid_velcity(sims, scene)
        self.apply_kinematic_constraints(sims, scene)
        self.compute_stress_strain(sims, scene)
