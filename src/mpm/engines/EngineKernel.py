...
'''
@ti.kernel
def kernel_momentum_apic_p2g(total_nodes: int, particleNum: int, node: ti.template(), particle: ti.template(), LnID: ti.template(), shapefn: ti.template(), node_size: ti.template()):
    for np in range(particleNum):
        if int(particle[np].active) == 1:
            bodyID = int(particle[np].bodyID)
            offset = np * total_nodes
    ...
    for np in range(particleNum):
        if int(particle[np].materialID) > 0 and int(particle[np].active) == 1:
            ...
            for i, j, k in ti.static(ti.ndrange(2, 2, 2)):
                ...
                #print(np, linear_grid_id, bodyID)
...
# ========================================================= #
#                #LXQ Grid to Particle (G2P)                    #
# ========================================================= #
@ti.kernel
def kernel_kinemaitc_g2p(total_nodes: int, alpha: float, localAlpha: float, dt: ti.template(), particleNum: int, node: ti.template(), particle: ti.template(), LnID: ti.template(), shapefn: ti.template(), node_size: ti.template()):
    # ti.block_local(dt)
    for np in range(particleNum):
        if int(particle[np].active) == 1:
            vPIC, vFLIP = ZEROVEC3f, ZEROVEC3f
            ...
            for ln in range(offset, offset + int(node_size[np])):
                ...
                vPIC += shape_mapping(shape_fn, velocity)
                vFLIP += shape_mapping(shape_fn, accleration) * dt[None]
            # LXQ vPIC is the grid velocity at t=n+1, vFLIP/dt is the grid accleration at t=n+1, particle damping acc = acc-a*vp
            vFLIP -= localAlpha*particle[np].v * dt[None]
            particle[np]._update_particle_state(dt, alpha, vPIC, vFLIP)
...
# ========================================================= #
#LXQ
#                 Compute Grid DTM normal                    #

#norm1 = vec3f(particle[np].normal)
#print(norm1)
# ========================================================= #
@ti.kernel
def kernel_calc_contact_normal_dtm(total_nodes: int, particleNum: int, node: ti.template(), particle: ti.template(), LnID: ti.template(), shapefn: ti.template(), node_size: ti.template(), is_rigid: ti.template()):
    for np in range(particleNum):
        if int(particle[np].active) == 1:
            bodyID = int(particle[np].bodyID)
            if is_rigid[bodyID] == 1:
                ...
                for ln in range(offset, offset + int(node_size[np])):
                    nodeID = LnID[ln]
                    #LXQ remove the comment in case you need to know the contacting nodes
                    #print(nodeID)
                    n_mass = shape_mapping(shapefn[ln], particle[np].m)
                    if (n_mass > node[nodeID, bodyID].grid_mass):
                        node[nodeID, bodyID].grid_mass = n_mass
                        #node[nodeID, bodyID].grid_normal = particle[np].normal
                        node[nodeID, bodyID]._update_nodal_np(np)
                        #node[nodeID, bodyID].grid_np = np
                        #print("nodeID, particleID")
                        #print(nodeID, bodyID, int(node[nodeID, bodyID].grid_np))
                        #np_temp = np
...
# ========================================================= #
@ti.kernel
def kernel_calc_friction_contact_dtm(cut_off: float, mu: float, position_offset: ti.types.vector(3, float), dt: ti.template(), t:float, is_rigid: ti.template(), node: ti.template(), particle: ti.template()):
    # ti.block_local(dt)
    for ng in range(node.shape[0]):
        bodyID1, bodyID2 = 0, 1
        ...
        if m1 > cut_off and m2 > cut_off:
            ...
            if is_rigid[bodyID1] == 0 and is_rigid[bodyID2] == 0:
                ...
                g_mass = m1 * dt[None]
                #norm = node[ng, bodyID1].grid_normal
                #norm = contact_normal_dtm(ng, bodyID1, node)
                np = int(node[ng, bodyID1].grid_np)
                norm = particle[np].normal
                #print(norm)
                #norm = [0.5, 0, 0.866]
                #print("nodeID, particleID")
                #print(ng, np)
                ...
                g_mass = m2 * dt[None]
                #norm = node[ng, bodyID2].grid_normal
