...
class myScene(object):
        ...
        if self.particle is None and sims.max_particle_num > 0:
            if sims.coupling or sims.neighbor_detection:
                if sims.solver_type == "Explicit":
                    ...
                    print('particle = ParticleCoupling')
    ...
    def activate_grid(self, sims: Simulation, contact, grid_level):
        ...
        if sims.contact_detection:
            ...
            #LXQ
            self.norm_method = DictIO.GetAlternative(contact, "NormMethod", "MassWeighted")
        ...
        if sims.stabilize == 'F-Bar Method' or sims.stress_smoothing:
    ...
    def set_boundary_conditions(self, sims: Simulation, boundary):
        ...
        #LXQ: possible used for two-grids?
        ...
        #
        inodes = self.element.get_boundary_nodes(start_point, end_point) #LXQ nodes within the rectangle
        #
        # LXQ material points along the boundaries
        ...
        if boundary_type == "VelocityConstraint":
            ...
            if sims.dimension == 2:
                for i in range(level, level + nlevel):
                    set_absorbing_contraint(self.absorbing_list, self.absorbing_boundary, inodes, i)
                copy_valid_absorbing_constraint(self.absorbing_list, self.absorbing_boundary)
            elif sims.dimension == 3:
                for i in range(level, level + nlevel):
                    set_absorbing_contraint(self.absorbing_list, self.absorbing_boundary, inodes, i)
                copy_valid_absorbing_constraint(self.absorbing_list, self.absorbing_boundary)

            print("Boundary Type: Absorbing Constraint")
            print("Start Point: ", start_point)
            print("End Point: ", end_point, '\n')
...
###############################################################################################################################
#LXQ: read velocity boundary conditions from txt file
###############################################################################################################################
                    boundary = str.split(boundary_constraints.readline())
                    node = int(boundary[0])
                    level = int(boundary[1])
                    direction = vec3f(float(boundary[2]), float(boundary[3]), float(boundary[4]))
                    velocity = vec3f(float(boundary[5]), float(boundary[6]), float(boundary[7]))
                    set_boundary_constraint_str(self.velocity_list, self.velocity_boundary, node, level, direction, velocity)
###############################################################################################################################
    ...
    def write_boundary_constraint(self, sims: Simulation, output_path):
        if not os.path.exists(output_path):
            os.mkdir(output_path)
        if self.velocity_list[0] > 0:
########################################################################################################################
#LXQ：output velocity boundary conditions to boundary_conditions.txt
########################################################################################################################
            output_path=output_path + f'/boundary_conditions.txt'
            with open(output_path, 'w') as f:
                print("VelocityConstraint", self.velocity_list[0], file=f)
                for ng in range(self.velocity_list[0]):
                    print(self.velocity_boundary.node[ng], self.velocity_boundary.level[ng],
                          ' '.join(map(str, self.velocity_boundary.fix_v[ng])), ' '.join(map(str, self.velocity_boundary.velocity[ng])), file=f)
########################################################################################################################
            #pass
    ...
    def find_boundary_points(self):
        return find_boundary_points_(self.particleNum[0], self.particle)
