...
class WriteFile:
    ...
    def MonitorContactGrid(self, sims: Simulation, scene: myScene):
        ...
        #LXQ
        #print(norm)
        #norm = scene.node.grid_normal.to_numpy()
        mass = scene.node.grid_np.to_numpy()
        #print(scene.node.grid_np)

        # self.VisualizeGrid(sims, coords, {"contact_force": (fcx, fcy, fcz)})
        np.savez(self.grid_path+f'/MPMGrid{sims.current_print:06d}', t_current=sims.current_time, dims=scene.element.gnum, coords=coords, contact_force=contact_force, normal=norm, mass=mass)
