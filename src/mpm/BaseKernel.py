...
#LXQ find the material point along the x, y, z boundaries
@ti.kernel
def find_boundary_points_(particleNum: int, particle: ti.template()):
    x_min, y_min, z_min = MThreshold, MThreshold, MThreshold
    x_max, y_max, z_max = 0., 0., 0.

    for np in range(particleNum):
        xpos = particle[np].x[0]
        ypos = particle[np].x[1]
        zpos = particle[np].x[2]

        ti.atomic_min(x_min, xpos)
        ti.atomic_min(y_min, ypos)
        ti.atomic_min(z_min, zpos)

        ti.atomic_max(x_max, xpos)
        ti.atomic_max(y_max, ypos)
        ti.atomic_max(z_max, zpos)

    for np in range(particleNum):
        xpos = particle[np].x[0]
        ypos = particle[np].x[1]
        zpos = particle[np].x[2]
        particle[np].boundary_state = [0,0,0]

        if xpos == x_min:  particle[np].boundary_state[0] = 1
        if xpos == x_max:  particle[np].boundary_state[0] = 2
        if ypos == y_min:  particle[np].boundary_state[1] = 1
        if ypos == y_max:  particle[np].boundary_state[1] = 2
        if zpos == z_min:  particle[np].boundary_state[2] = 1
        if zpos == z_max:  particle[np].boundary_state[2] = 2
