...
#########################################################################################################
#LXQ add boundary from string
#########################################################################################################
@ti.kernel
def set_boundary_constraint_str(lists: ti.types.ndarray(), constraint: ti.template(), node: int, level: int, direction: ti.types.vector(3, float), velocity: ti.types.vector(3, float)):# type: ignore
    locate = lists[0]
    constraint[locate].set_boundary_condition(node, level, direction, velocity)
    #print(node, level, direction, velocity)
...
def set_absorbing_contraint(lists: ti.types.ndarray(), constraint: ti.template(), inodes: ti.types.ndarray(), level: int):
    start_index = lists[0]
    #print('absorbing_list=', lists[0], 'inodes.shape=', inodes.shape[0])
    for offset in range(inodes.shape[0]):
        locate = start_index + offset
        find_pre_location(start_index, level, inodes[offset], constraint, locate)
        constraint[locate].set_boundary_condition(inodes[offset], level)
    lists[0] += inodes.shape[0]
    ...
    for nboundary in range(lists):
        ...
        if node[nodeID, grid_level].m > cut_off:
            ...
            if is_rigid[grid_level] == 0:
                ...
                #print(nodeID, grid_level, prescribed_velocity)
            else:
                node[nodeID, grid_level].rigid_body_velocity_constraint(fix_v, unfix_v)
                #print("breakingpoints at apply_velocity_constraint")
...
def apply_contact_velocity_constraint(cut_off: float, lists: int, constraints: ti.template(), node: ti.template()):
    for nboundary in range(lists):
        nodeID = constraints[nboundary].node
        #print("apply_contact_velocity_constraint", nodeID)
        grid_level = int(constraints[nboundary].level)
...
def apply_absorbing_constraint(lists: int, constraints: ti.template(), material: ti.template(), node: ti.template(), extra_node: ti.template()):
    for nboundary in range(lists):
        ...
        a = constraints[nboundary].alpha #LXQ class AbsorbingConstraint
        b = constraints[nboundary].beta #LXQ
        position_type = constraints[nboundary].position_type #LXQ

        mass = node[nodeID, grid_level].m #LXQ class Nodes
        if node[nodeID, grid_level].m > Threshold:
            print('absorbing', 'nodeID=', nodeID, 'grid_level=', grid_level,  'mass=', mass)
...
# LXQ
@ti.kernel
def apply_absorbing_constraint_p(particleNum: int, particle: ti.template(), material: ti.template()):
    materialID = 1
    dir = 2 #0,1,2

    mat = material.matProps[materialID]  # Cache the material properties lookup
    cd = ((mat.bulk + 4.0 * mat.shear / 3.0) / mat.density) ** 0.5 #cd  WaveSpeed
    cs = (mat.shear/mat.density)** 0.5 #cs  ShearWaveSpeed

    for np in range(particleNum):
        if dir == 0:
            if particle[np].boundary_state[0] != 0:
                particle[np].boundary_force[0] = particle[np].m * cd * particle[np].v[0] / (2. * particle[np].psize[0])
                particle[np].boundary_force[1] = particle[np].m * cs * particle[np].v[1] / (2. * particle[np].psize[0])
                particle[np].boundary_force[2] = particle[np].m * cs * particle[np].v[2] / (2. * particle[np].psize[0])
                particle[np].traction -= particle[np].boundary_force
        elif dir == 1:
            if particle[np].boundary_state[1] == 2:
                particle[np].boundary_force[0] = particle[np].m * cs * particle[np].v[0] / (2. * particle[np].psize[1])
                particle[np].boundary_force[1] = particle[np].m * cd * particle[np].v[1] / (2. * particle[np].psize[1])
                particle[np].boundary_force[2] = particle[np].m * cs * particle[np].v[2] / (2. * particle[np].psize[1])
                particle[np].traction -= particle[np].boundary_force
        elif dir == 2:
            if particle[np].boundary_state[2] == 2:
                particle[np].boundary_force[0] = particle[np].m * cs * particle[np].v[0] / (2. * particle[np].psize[2])
                particle[np].boundary_force[1] = particle[np].m * cs * particle[np].v[1] / (2. * particle[np].psize[2])
                particle[np].boundary_force[2] = particle[np].m * cd * particle[np].v[2] / (2. * particle[np].psize[2])
                print(particle[np].traction, particle[np].boundary_force[2])
                #particle[np].traction -= particle[np].boundary_force
...
def apply_traction_constraint(lists: int, constraints: ti.template(), node: ti.template()):
    for nboundary in range(lists):
        nodeID = constraints[nboundary].node
        grid_level = int(constraints[nboundary].level)
        mass = node[nodeID, grid_level].m
        #print('traction', 'nodeID=', nodeID, 'mass=', mass)
        if node[nodeID, grid_level].m > Threshold:
