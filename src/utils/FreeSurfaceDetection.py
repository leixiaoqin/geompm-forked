...
@ti.kernel
def assign_particle_free_surface(particleNum: int, particle: ti.template(), material: ti.template()):
    density_ratio_tolerance = 0.76
    for np in range(particleNum):
        if int(particle[np].active) == 1:
            is_free_surface = 0
            materialID = int(particle[np].materialID)
            if particle[np].mass_density / material[materialID].density <= density_ratio_tolerance:
                is_free_surface = 1
                #print('is_free_surface', np)
            particle[np].free_surface = ti.u8(is_free_surface)



@ti.kernel
def find_boundary_direction_by_geometry(igrid_size: float, cnum: ti.types.vector(3, int), particleNum: int, particle: ti.template(), particleID: ti.template(), particle_current: ti.template(), particle_count: ti.template()):
    for np in range(particleNum):
        normal = ZEROVEC3f
...
@ti.kernel
def find_free_surface_by_geometry(igrid_size: float, cnum: ti.types.vector(3, int), particleNum: int, particle: ti.template(), particleID: ti.template(), particle_current: ti.template(), particle_count: ti.template()):
    for np in range(particleNum):
        if particle[np].free_surface == ti.u8(1) and int(particle[np].active) == 1:
            p_coord = particle[np].x
            grid_idx = ti.floor(p_coord * igrid_size, int)
